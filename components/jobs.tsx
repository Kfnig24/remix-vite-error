...
export const JobCategorie = ({ title, jobs, type }: { title: string, type: 'client' | 'teen', jobs: ({ title: string, description: string, date: string, id: number, categorie: string } | undefined | null)[] | null }) => {
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <div className="flex overflow-x-auto space-x-8 h-60 p-4 scrollbar-hide md:scrollbar-default">
                {jobs.map(job => <JobCard type={type} key={job?.id} job={job} />)}
            </div>
        </div>
    )
}

export const JobCard = ({ job, type }: { type: 'client' | 'teen', job: { title: string, description: string, date: string, id: number, categorie: string } | undefined | null }) => {
    if (!job) return null

    return (
        <div className="p-4 flex-shrink-0 rounded-md shadow-md w-96 h-48 text-gray-800 flex flex-col justify-between">
            <div className="flex flex-row justify-between items-center">
                <Link href={`/${type}/jobs/${job.id}`}>
