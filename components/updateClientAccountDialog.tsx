...
import { deleteUser, updateClientAccount } from "@/lib/actions/user"
import { toast } from "./ui/use-toast"

export const UpdateAccountDialogTrigger = ({ children }: { children: React.ReactNode }) => {
    return (
...
const UpdateClientAccountDialogContent = ({ client_data }: { client_data: { first_name: string, last_name: string, address: string, city: string, email: string } }) => {
    ...
    const onSubmit = async (values: z.infer<typeof clientFormSchema>) => {
        const error = await updateClientAccount(values)
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input placeholder="Ville" className="p-6 text-md" {...field} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
...
const UpdateClientAccountDialogWrapper = ({ children, client_data }: { children: React.ReactNode, client_data: { first_name: string, last_name: string, address: string, city: string, email: string } }) => {
    return (
        <Dialog>
            {children}
            <UpdateClientAccountDialogContent client_data={client_data} />
        </Dialog>
    )
}
export default UpdateClientAccountDialogWrapper